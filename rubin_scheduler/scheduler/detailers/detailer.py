        ...
        for i in np.arange(obs_array.size):
            ...
            if self.append:
                if obs_array[self.field_for_label][i] == "":
                    ...
                    obs_array[self.field_for_label][i] = obs_array[self.field_for_label][i] + self.seperator + result
            else:
                obs_array[self.field_for_label][i] = result
        return obs_array
...
class LabelRegionsAndDDFs(BaseDetailer):
    ...
    """
    def __init__(self, label_array=None, field_for_label="target_name",
                 camera="LSST", append=True,
                 ddf_location=None, seperator=", ", match_radius=2.0):
        self.label_detailer = LabelRegionDetailer(label_array=label_array,
                                                  field_for_label=field_for_label, 
                                                  camera=camera, append=append, seperator=seperator)
        self.ddf_detailer = LabelDDFDetailer(ddf_location=ddf_location, field_for_label=field_for_label,
                                             append=append, match_radius=match_radius,
                                             seperator=seperator)
