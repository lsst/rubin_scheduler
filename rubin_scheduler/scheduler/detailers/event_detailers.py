...
class BandPickToODetailer(BaseDetailer):
    """Swap which filters are used based on information in a
    ToO event.
    ...
    distance_limit : `float`
        For distances greater(?) than distance_limit,
    ...
    def __init__(self, band_start="i", band_end="z", distance_limit=10.0, check_mounted=True):
    ...
    def __call__(self, observations, conditions, target_o_o=None):
        ...
        if hasattr(target_o_o, "posterior_distance"):
            if target_o_o.posterior_distance is not None:
                if target_o_o.posterior_distance > self.distance_limit:
